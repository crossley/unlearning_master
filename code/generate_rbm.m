function [nc_rbm_vars] = generate_rbm(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/uRB30' num2str(i) '.txt']);
...
nc_rbm_vars = cell(1,4);
nc_rbm_vars{1} = learning_curve;
nc_rbm_vars{2} = std_err_curve;
nc_rbm_vars{3} = sub_accuracy;
nc_rbm_vars{4} = excluded_subs;
nc_rbm_vars{5} = savings_record;
nc_rbm_vars{6} = intervention_record;
nc_rbm_vars{7} = mean(accuracy_record);