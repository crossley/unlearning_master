function [renewal_AAB_vars] = generate_renewal_AAB(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/frcB' num2str(i) '.dat']);
...
renewal_AAB_vars = cell(1,4);
renewal_AAB_vars{1} = learning_curve;
renewal_AAB_vars{2} = std_err_curve;
renewal_AAB_vars{3} = sub_accuracy;
renewal_AAB_vars{4} = excluded_subs;
renewal_AAB_vars{5} = savings_record;
renewal_AAB_vars{6} = intervention_record;