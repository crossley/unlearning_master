function [nc_40_vars] = generate_nc_40(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/nc40' num2str(i) '.dat']);
...
nc_40_vars = cell(1,4);
nc_40_vars{1} = learning_curve;
nc_40_vars{2} = std_err_curve;
nc_40_vars{3} = sub_accuracy;
nc_40_vars{4} = excluded_subs;
nc_40_vars{5} = savings_record;
nc_40_vars{6} = intervention_record;
nc_40_vars{7} = mean(accuracy_record);