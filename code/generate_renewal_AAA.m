function [renewal_AAA_vars] = generate_renewal_AAA(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/frc' num2str(i) '.dat']);
...
renewal_AAA_vars = cell(1,4);
renewal_AAA_vars{1} = learning_curve;
renewal_AAA_vars{2} = std_err_curve;
renewal_AAA_vars{3} = sub_accuracy;
renewal_AAA_vars{4} = excluded_subs;
renewal_AAA_vars{5} = savings_record;
renewal_AAA_vars{6} = intervention_record;