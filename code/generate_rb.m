function [nc_rb_vars] = generate_rb(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/uRB10' num2str(i) '.txt']);
...
nc_rb_vars = cell(1,4);
nc_rb_vars{1} = learning_curve;
nc_rb_vars{2} = std_err_curve;
nc_rb_vars{3} = sub_accuracy;
nc_rb_vars{4} = excluded_subs;
nc_rb_vars{5} = savings_record;
nc_rb_vars{6} = intervention_record;
nc_rb_vars{7} = mean(accuracy_record);