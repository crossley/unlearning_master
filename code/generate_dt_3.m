function [dt_3_vars] = generate_dt_3(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/unl43' num2str(i) '.dat']);
    ...
    if mean(feedback(201:250)) < exclusion_criteria_acc || mean(dt_feedback(251:550)) < exclusion_criteria_dt
        ...
        dual_task_record = [dual_task_record; mean(dt_feedback(251:550))];
    end
...
dt_3_vars = cell(1,4);
dt_3_vars{1} = learning_curve;
dt_3_vars{2} = std_err_curve;
dt_3_vars{3} = sub_accuracy;
dt_3_vars{4} = excluded_subs;
dt_3_vars{5} = savings_record;
dt_3_vars{6} = intervention_record;
dt_3_vars{7} = dual_task_record;
dt_3_vars{8} = anova_savings;