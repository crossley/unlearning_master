function [dt_2_vars] = generate_dt_2(path, sub_num, block_struct, exclusion_criteria)
...
exclusion_criteria_dt = 0; % no dual-task in control condition

for i = sub_num
        
    data = dlmread([path '/un4bc' num2str(i) '.txt']);
    ...
    % color = lines(4);
    % figure, hold;
    % plot(x(cat == 1),y(cat == 1),'.','color',color(1,:))
    % plot(x(cat == 2),y(cat == 2),'.','color',color(2,:))
    % plot(x(cat == 3),y(cat == 3),'.','color',color(3,:))
    % plot(x(cat == 4),y(cat == 4),'.','color',color(4,:))
    ...
    if mean(feedback(201:250)) < exclusion_criteria_acc || mean(dt_feedback(251:450)) < exclusion_criteria_dt
        ...
        dual_task_record = [dual_task_record; mean(dt_feedback(251:450))];
    end
...
dt_2_vars = cell(1,4);
dt_2_vars{1} = learning_curve;
dt_2_vars{2} = std_err_curve;
dt_2_vars{3} = sub_accuracy;
dt_2_vars{4} = excluded_subs;
dt_2_vars{5} = savings_record;
dt_2_vars{6} = intervention_record;
dt_2_vars{7} = dual_task_record;
dt_2_vars{8} = anova_savings;