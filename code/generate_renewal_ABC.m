function [renewal_ABC_vars] = generate_renewal_ABC(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/frcC' num2str(i) '.dat']);
...
renewal_ABC_vars = cell(1,4);
renewal_ABC_vars{1} = learning_curve;
renewal_ABC_vars{2} = std_err_curve;
renewal_ABC_vars{3} = sub_accuracy;
renewal_ABC_vars{4} = excluded_subs;
renewal_ABC_vars{5} = savings_record;
renewal_ABC_vars{6} = intervention_record;