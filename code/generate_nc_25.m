function [nc_25_vars] = generate_nc_25(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num_1
    ...
    data = dlmread([path '/unl1' num2str(i) '.dat']);
    ...
    if mean(feedback(201:300)) < exclusion_criteria_acc
        excluded_subs_1 = [excluded_subs_1 i];
    else
...
for i = sub_num_2
    ...
    data = dlmread([path '/ext5_' num2str(i) '.txt']);
    ...
    if mean(feedback(201:300)) < exclusion_criteria_acc
        excluded_subs_2 = [excluded_subs_2 i];
    else
        accuracy_record = [accuracy_record; feedback'];
    end
...
sub_num_1 = setdiff(sub_num_1,excluded_subs_1);
sub_num_2 = setdiff(sub_num_2,excluded_subs_2);

excluded_subs = [excluded_subs_1 excluded_subs_2];
sub_num = [sub_num_1 sub_num_2];
...
nc_25_vars = cell(1,4);
nc_25_vars{1} = learning_curve;
nc_25_vars{2} = std_err_curve;
nc_25_vars{3} = sub_accuracy;
nc_25_vars{4} = excluded_subs;
nc_25_vars{5} = savings_record;
nc_25_vars{6} = intervention_record;
nc_25_vars{7} = mean(accuracy_record);