function [nc_63_vars] = generate_nc_63(path, sub_num, block_struct, exclusion_criteria)
...
for i = sub_num
    ...
    data = dlmread([path '/nc63' num2str(i) '.dat']);
...
nc_63_vars = cell(1,4);
nc_63_vars{1} = learning_curve;
nc_63_vars{2} = std_err_curve;
nc_63_vars{3} = sub_accuracy;
nc_63_vars{4} = excluded_subs;
nc_63_vars{5} = savings_record;
nc_63_vars{6} = intervention_record;
nc_63_vars{7} = mean(accuracy_record);