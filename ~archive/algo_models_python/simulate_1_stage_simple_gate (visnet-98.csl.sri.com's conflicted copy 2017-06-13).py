...
def simulate(params, stimuli, num_simulations):
    ...
    for simulation in xrange(num_simulations):
        for trial in xrange(num_trials):
            ...
            # update gate
            # TODO: the smaller this value gets, the less it is updated... the pause issue
            # if delta < 0:
            #     w_gate += alpha_gate * logistic(w_gate, k_gate, x_mid_gate) * delta * w_gate
            # else:
            #     w_gate += alpha_gate * logistic(w_gate, k_gate, x_mid_gate) * delta * (1 - w_gate)
            # w_gate = cap(w_gate)
            if delta < 0:
