...
cats <- data[1:900, .(V2, V3, V4)]
setnames(cats, c('cat', 'x', 'y'))
ggplot(cats, aes(x=x, y=y, colour=factor(cat))) +
  geom_point()

d <- data[exc_acc > .4]
dd <- d[, .(mean(acc), sd(acc)/sqrt(length(unique(sub)))), .(cnd1, cnd2, block)]
dd[, cnd1 := factor(cnd1, levels=c('RF(.25)', 'Mixed', 'RF(.40)'))]

ggplot(dd, aes(x=block, y=V1, colour=factor(cnd2))) +
  geom_line() +
...
ggsave('crossleyetal2013.pdf', width = 8, height = 3.5)

## stats
d <- data[exc_acc > .4]
dd <- d[, .(mean(acc)), .(sub, cnd1, cnd2, block)]
dd[, cnd1 := factor(cnd1, levels=c('RF(.25)', 'Mixed', 'RF(.40)'))]

## compare acquisition phase
x <- dd[cnd1 == 'RF(.40)' & cnd2 == 'Relearning' & block %in% 1:12, V1]
y <- dd[cnd1 == 'Mixed' & cnd2 == 'Relearning' & block %in% 1:12, V1]
t.test(x, y, alternative = 'two.sided')

## compare reacquisition phase - Relearning
x <- dd[cnd1 == 'RF(.40)' & cnd2 == 'Relearning' & block %in% 25:36, V1]
y <- dd[cnd1 == 'Mixed' & cnd2 == 'Relearning' & block %in% 25:36, V1]
t.test(x, y, alternative = 'two.sided')

## compare reacquisition phase - New Learning
x <- dd[cnd1 == 'RF(.40)' & cnd2 == 'New Learning' & block %in% 25:36, V1]
y <- dd[cnd1 == 'Mixed' & cnd2 == 'New Learning' & block %in% 25:36, V1]
t.test(x, y, alternative = 'two.sided')
