...
pdf("savings_3d.pdf",width=15,height=10) 
...
cnd <- 1
acq <- setnames(data[condition==cnd, mean(accuracy[trial %in% acq_ind]), .(condition, subject)],'V1','acc')$acc
ext <- setnames(data[condition==cnd, mean(accuracy[trial %in% ext_ind]), .(condition, subject)],'V1','acc')$acc
reac <- setnames(data[condition==cnd, mean(accuracy[trial %in% reac_ind]), .(condition, subject)],'V1','acc')$acc

xy <- data.frame(acq,ext,reac=0)
xz <- data.frame(acq,ext=1,reac)
yz <- data.frame(acq=0,ext,reac)
xyz <- rbind(xy,xz,yz)

savings <- as.character(reac-acq > 0)
savings[which(savings=="TRUE")] <- "red"
savings[which(savings=="FALSE")] <- "blue"
...
exc_crit <- 0
exc_subs <- savings[savings < exc_crit, .(condition,subject)]
...
acq <- setnames(data[, mean(accuracy[trial %in% 201:300]), .(condition, subject)],'V1','acq')


pdf("fig_acquisition.pdf",width=10,height=7) 
...
boxplot(acq ~ condition, data=acq, names=c('1','2','3','4','Control'), xlab='Condition', ylab='Accuracy')
points(acq ~ condition, data=acq)
# abline(h=exc_crit, col='red')
...
hist(savings[condition==1,savings,subject]$savings, 5)
hist(savings[condition==2,savings,subject]$savings, 5)
hist(savings[condition==3,savings,subject]$savings, 5)
hist(savings[condition==4,savings,subject]$savings, 5)
hist(savings[condition==5,savings,subject]$savings, 5)
hist(savings[condition==6,savings,subject]$savings, 5)
