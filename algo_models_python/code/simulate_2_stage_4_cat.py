...
def simulate(stimuli, num_simulations, params):
    ...
    w_C_1 = np.random.normal(w_base, w_noise, dim**2)
    w_D_1 = np.random.normal(w_base, w_noise, dim**2)
    w_A_2 = np.random.normal(w_base, w_noise, 2)
    w_B_2 = np.random.normal(w_base, w_noise, 2)
    w_C_2 = np.random.normal(w_base, w_noise, 2)
    w_D_2 = np.random.normal(w_base, w_noise, 2)
    ...
    context = [[w_A_1, w_B_1, w_C_1, w_D_1, w_A_2, w_B_2, w_C_2, w_D_2]]
    current_context = 0
    ...
    for simulation in xrange(num_simulations):
        for trial in xrange(num_trials):
            ...
            for x in xrange(len(context)):
                ...
                act_C_1[x] = np.inner(vis_act, context[x][2])
                act_D_1[x] = np.inner(vis_act, context[x][3])

                # add noise to output units
                act_A_1[x] += np.random.normal(w_base, w_noise, 1)
                act_B_1[x] += np.random.normal(w_base, w_noise, 1)
                act_C_1[x] += np.random.normal(w_base, w_noise, 1)
                act_D_1[x] += np.random.normal(w_base, w_noise, 1)

                # Implement strong lateral inhibition --- stage 1
                act_array_1 = np.array([act_A_1[x][0], act_B_1[x][0],
                                        act_C_1[x][0], act_D_1[x]])
                act_sort_ind_1 = np.argsort(act_array_1)
                act_max_1 = act_sort_ind_1[3] + 1

                act_A_1[x] = act_A_1[x] if act_max_1[x] == 1 else 0.0
                act_B_1[x] = act_B_1[x] if act_max_1[x] == 2 else 0.0
                act_C_1[x] = act_C_1[x] if act_max_1[x] == 3 else 0.0
                act_D_1[x] = act_D_1[x] if act_max_1[x] == 4 else 0.0
                ...
                act_C_2 = 0.0
                act_D_2 = 0.0
                act_A_2 = np.inner([act_A_1, act_B_1, act_C_1, act_D_1], w_A_2)
                act_B_2 = np.inner([act_A_1, act_B_1, act_C_1, act_D_1], w_B_2)
                act_C_2 = np.inner([act_A_1, act_B_1, act_C_1, act_D_1], w_C_2)
                act_D_2 = np.inner([act_A_1, act_B_1, act_C_1, act_D_1], w_D_2)

                # Implement strong lateral inhibition --- stage 2
                act_array_2 = np.array([act_A_2[x][0], act_B_2[x][0],
                                        act_C_2[x][0], act_D_2[x]])
                act_sort_ind_2 = np.argsort(act_array_2)
                act_max_2 = act_sort_ind_2[3] + 1

                act_A_2[x] = act_A_2[x] if act_max_2[x] == 1 else 0.0
                act_B_2[x] = act_B_2[x] if act_max_2[x] == 2 else 0.0
                act_C_2[x] = act_C_2[x] if act_max_2[x] == 3 else 0.0
                act_D_2[x] = act_D_2[x] if act_max_2[x] == 4 else 0.0
                ...
                # compute discrim via max act diff
                discrim[x] = (act_array_2[3] - act_array_2[2]) / act_array_2[3]
                ...
                if r[x] == 1.0:
                    x_obs_pos[x] = np.abs(discrim[x])
                    x_obs_neg[x] = 0.0
                else:
                    x_obs_pos[x] = 0.0
                    x_obs_neg[x] = np.abs(discrim[x])
            ...
            for i in xrange(0, dim**2):
                if delta < 0:
                    weight_change_A = alpha_actor * vis_act[i] * act_A_1[current_context] * delta * context[current_context][0][i]
                    context[current_context][0][i] += weight_change_A

                    weight_change_B = alpha_actor * vis_act[i] * act_B_1[current_context] * delta * context[current_context][1][i]
                    context[current_context][1][i] += weight_change_B

                    weight_change_C = alpha_actor * vis_act[i] * act_C_1[current_context] * delta * context[current_context][2][i]
                    context[current_context][2][i] += weight_change_C

                    weight_change_D = alpha_actor * vis_act[i] * act_D_1[current_context] * delta * context[current_context][3][i]
                    context[current_context][3][i] += weight_change_D
                else:
                    weight_change_A = alpha_actor * vis_act[i] * act_A_1[current_context] * delta * (1 - context[current_context][0][i])
                    context[current_context][0][i] += weight_change_A

                    weight_change_B = alpha_actor * vis_act[i] * act_B_1[current_context] * delta * (1 - context[current_context][1][i])
                    context[current_context][1][i] += weight_change_B

                    weight_change_C = alpha_actor * vis_act[i] * act_C_1[current_context] * delta * (1 - context[current_context][2][i])
                    context[current_context][2][i] += weight_change_C

                    weight_change_D = alpha_actor * vis_act[i] * act_D_1[current_context] * delta * (1 - context[current_context][3][i])
                    context[current_context][3][i] += weight_change_D
            ...
            act_1 = [act_A_1, act_B_1, act_C_1, act_D_1]
            for i in xrange(0,4):
                if delta < 0 :
                    w_A_2[i] += alpha_actor_2 * act_1[i]  * act_A_2 * delta * w_A_2[i]
                    w_B_2[i] += alpha_actor_2 * act_1[i] * act_B_2 * delta * w_B_2[i]
                    w_C_2[i] += alpha_actor_2 * act_1[i] * act_C_2 * delta * w_C_2[i]
                    w_D_2[i] += alpha_actor_2 * act_1[i] * act_D_2 * delta * w_D_2[i]
                else:
                    w_A_2[i] += alpha_actor_2 * act_1[i] * act_A_2 * delta * (1 - w_A_2[i])
                    w_B_2[i] += alpha_actor_2 * act_1[i] * act_B_2 * delta * (1 - w_B_2[i])
                    w_C_2[i] += alpha_actor_2 * act_1[i] * act_C_2 * delta * (1 - w_C_2[i])
                    w_D_2[i] += alpha_actor_2 * act_1[i] * act_D_2 * delta * (1 - w_D_2[i])

                context[current_context][4][i] = cap(context[current_context][4][i])
                context[current_context][5][i] = cap(context[current_context][5][i])
                context[current_context][6][i] = cap(context[current_context][6][i])
                context[current_context][7][i] = cap(context[current_context][7][i])
